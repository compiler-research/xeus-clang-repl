...
static bool ProcessCode(clang::Interpreter& Interp, const std::string& code,
  ...
  if (code.substr(0, 6) == "python") {
    PyRun_SimpleString(
        ...
        "globals().copy()\nfirst_dict_ints={k:a[k] for k in set(a) if "
        "type(a[k]) == int}\nfirst_dict_lists={k:a[k] for k in set(a) if "
        "type(a[k]) == list}");
    ExecSimpleCommand(code.substr(6));
    // PyRun_SimpleString("tmp = globals().copy()\nvars = [f'int {k} = {v};' for
    // k,v in tmp.items() if type(v) == int and not k.startswith('_') and
    // k!='tmp' and k!='In' and k!='Out' and k!='sys' and not hasattr(v,
    // '__call__')]\nprint(vars)"); PyRun_SimpleString("b =
    // globals().copy()\nnew_ints = ' '.join([f'int {k} = {b[k]};' for k in
    // set(b) - set(first_dict) if type(b[k]) == int])\nprint('new_ints: ',
    // new_ints)");

    // transfer ints utility
    PyRun_SimpleString(
        "def getNewInts():\n    b = globals().copy()\n    new_ints = ' "
        "'.join([f'int {k} = {b[k]};' for k in set(b) - set(first_dict_ints) "
        "if type(b[k]) == int])\n    return new_ints");
    ...
    PyRun_SimpleString("def getNewLists():\n    l = globals().copy()\n    "
                       ...
                       "').replace(')',']')\n    return new_lists");
    PyObject* lists_result = PyObject_CallFunction(
        PyDict_GetItemString(gMainDict, "getNewLists"), 0);
    if (!lists_result) {
      ...
      if (code.substr(0, 3) == "int") {
        ...
        for (clang::Decl* D : PTU->TUPart->decls()) {
          if (clang::VarDecl* VD = llvm::dyn_cast<clang::VarDecl>(D)) {
            ...
            void* AddrVP = (void*)*Addr;
            UpdatePythonDictVariableVector(Name.c_str(),
                                           *(std::vector<int>*)AddrVP);
          }
        }
      }
...
interpreter::interpreter(int argc, const char* const* argv)
    : m_interpreter(
          std::move(createInterpreter(Args(argv + 2, argv + argc - 3)))),
      //          m_input_validator(),
...
nl::json interpreter::execute_request_impl(int execution_counter,
  ...
  for (const auto& block : blocks) {
    ...
    std::string err;
    try {
      // compilation_result = m_interpreter.process(block, &output, nullptr,
      // true);
