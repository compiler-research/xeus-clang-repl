...
struct node_predicate {
  ...
  bool operator()(pugi::xml_node node) const {
    return static_cast<std::string>(node.attribute("kind").value()) == kind &&
           static_cast<std::string>(node.child("name").child_value()) ==
               child_value;
  }
};
...
struct class_member_predicate {
  ...
  std::string get_filename(pugi::xml_node node) {
    for (pugi::xml_node child : node.children()) {
      if (static_cast<std::string>(child.attribute("kind").value()) == kind &&
          static_cast<std::string>(child.child("name").child_value()) ==
              child_value) {
  ...
  bool operator()(pugi::xml_node node) {
    auto parent =
        (static_cast<std::string>(node.attribute("kind").value()) == "class" ||
         static_cast<std::string>(node.attribute("kind").value()) ==
             "struct") &&
        static_cast<std::string>(node.child("name").child_value()) ==
            class_name;
    auto found = false;
...
std::string find_type(const std::string& expression,
  ...
  // Temporarily dismissing all std::cerr and std::cout resulting from
  // `interpreter.process`
...
static nl::json read_tagconfs(const char* path) {
  ...
  while (item != nullptr) {
    ...
    if (item->d_type == DT_REG) {
      ...
      if (fname.find(extension, (fname.length() - extension.length())) !=
          std::string::npos) {
...
void inspect(const std::string& code, nl::json& kernel_res,
  ...
  if (std::regex_search(to_inspect, method, std::regex(R"((.*)\.(\w*)$)"))) {
    ...
    if (!typename_.empty()) {
      for (nl::json::const_iterator it = tagconfs.cbegin();
           it != tagconfs.cend(); ++it) {
        url = it->at("url");
    ...
    for (nl::json::const_iterator it = tagconfs.cbegin(); it != tagconfs.cend();
         ++it) {
      url = it->at("url");
      tagfile = it->at("tagfile");
      std::string filename = tagfiles_dir + "/" + tagfile;
...
public:
  ...
  xintrospection(cling::Interpreter& p) : m_interpreter{p} {
    pattern = spattern;
  }
