//===---------- xbuffer.hpp - Stream buffer handling ------------*- C++ -*-===//
//
...
//===----------------------------------------------------------------------===//
//
//  This file stream buffer handling facilities.
//
//===----------------------------------------------------------------------===//
...
public:
  ...
  xoutput_buffer(callback_type callback) : m_callback(std::move(callback)) {}
...
public:
  ...
  xinput_buffer(callback_type callback)
      : m_callback(std::move(callback)), m_value() {
