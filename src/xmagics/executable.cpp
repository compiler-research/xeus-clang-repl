...
#include "clang/Frontend/Transaction.h"
...
std::string executable::generate_fns(const std::string& cell, std::string& main,
...
class FindTopLevelDecls : public clang::RecursiveASTVisitor<FindTopLevelDecls> {
public:
...
bool executable::generate_obj(std::string& ObjectFile, bool EnableDebugInfo) {
  ...
  EmitBackendOutput(CI->getDiagnostics(), HeaderSearchOpts, CodeGenOpts,
                    CI->getTargetOpts(), CI->getLangOpts(), DataLayout,
                    CG->GetModule(), clang::Backend_EmitObj, std::move(OS));
  return true;
}
...
bool executable::generate_exe(const std::string& ObjectFile,
  ...
  const llvm::StringRef* Redirects[] = {nullptr, &OutputFileStr, &ErrorFileStr};

  // Finally run the linker.
  int ret =
      llvm::sys::ExecuteAndWait(Compiler, Args.data(), nullptr, Redirects);
...
void executable::operator()(const std::string& line, const std::string& cell) {
  ...
  if (ExeFile.empty()) {
    std::cerr << "UsageError: "
              << "the following arguments are required: filename" << std::endl;
    return;
  }
  ...
  if (result != clang::Interpreter::kSuccess) {
    return;
  }

  // Now declare main() function.
  clang::Transaction* t = nullptr;
  result = m_interpreter.declare(main, &t);
  if (result != clang::Interpreter::kSuccess || t == nullptr) {
    return;
  }
  ...
  struct Unloader {
    ...
    clang::Transaction& m_transaction;
    Unloader(clang::Interpreter& i, clang::Transaction& t)
        : m_interpreter(i), m_transaction(t) {}
    ~Unloader() { m_interpreter.unload(m_transaction); }
  } unloader(m_interpreter, *t);

  // Enable debug information if user requested -g in the linker options.
  bool EnableDebugInfo = (std::find(LinkerOptions.begin(), LinkerOptions.end(),
                                    "-g") != LinkerOptions.end());
  if (EnableDebugInfo) {
  ...
  // the linker options.
  bool SanitizeThread = (std::find(LinkerOptions.begin(), LinkerOptions.end(),
                                   "-fsanitize=thread") != LinkerOptions.end());
  auto& SanitizeOpts = m_interpreter.getCI()->getLangOpts().Sanitize;
  if (SanitizeThread) {
    std::cout << "Enabling instrumentation for ThreadSanitizer" << std::endl;
