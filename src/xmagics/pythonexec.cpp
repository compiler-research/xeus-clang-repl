...
namespace xcpp
{
    ...
    void pythonexec::execute(std::string& line, std::string& cell)
    {
        ...
        PyRun_SimpleString("globals_copy_lists = globals().copy()\nfirst_dict_ints={k:globals_copy_lists[k] for k in set(globals_copy_lists) if type(globals_copy_lists[k]) == int}\nfirst_dict_lists={k:globals_copy_lists[k] for k in set(globals_copy_lists) if type(globals_copy_lists[k]) == list}");
    ...
    void pythonexec::update_python_dict_var_vector(const char *name, std::vector<int> &data) {
        if (!gMainDict) startup();
