...
namespace xcpp {
timeit::timeit(cling::Interpreter* p) : m_interpreter(p) {
  cling::Interpreter::CompilationResult compilation_result;
  compilation_result = m_interpreter->process("#include <chrono>");
  std::string init_timeit =
...
xoptions timeit::get_options() {
  ...
  options.add_options()("n,number",
                        "execute the given statement n times in a loop. If "
                        "this value is not given, a fitting value is chosen",
                        cxxopts::value<std::size_t>())(
      "r,repeat", "repeat the loop iteration r times and take the best result",
...
std::string timeit::inner(std::size_t number, const std::string& code) const {
  ...
  timeit_code +=
      "for (std::size_t _i = 0; _i < " + std::to_string(number) + "; ++_i) {\n";
  timeit_code += "   " + code + "\n";
  timeit_code += "}\n";
...
std::string timeit::_format_time(double timespan, std::size_t precision) const {
  ...
  if (timespan > 0.0) {
    order = std::min(
        -static_cast<int>(std::floor(std::floor(std::log10(timespan)) / 3)), 3);
  } else {
    order = 3;
  }
...
void timeit::execute(std::string& line, std::string& cell) {
  ...
  std::size_t number =
      (result.count("n")) ? result["n"].as<std::size_t>() : 0ul;
  ...
  try {
    if (number == 0ul) {
      for (std::size_t n = 0; n < 10; ++n) {
        ...
        compilation_result =
            m_interpreter->process(timeit_code.c_str(), &output);
    ...
    std::cout << _format_time(mean, precision) << " +- "
